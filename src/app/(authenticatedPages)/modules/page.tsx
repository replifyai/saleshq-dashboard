  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Calculate stats from the tree
        let totalModules = 0;
        let totalPdfs = 0;
        let totalQuizzes = 0;

        const countResources = async (nodes: any[]) => {
          for (const node of nodes) {
            totalModules++;
            
            // Get module details to count resources
            try {
              const moduleDetails = await moduleApi.getModule(node.id);
              const files = moduleDetails.data.files || [];
              totalPdfs += files.filter((f: any) => f.type === 'pdf').length;
              totalQuizzes += files.filter((f: any) => f.type === 'quiz').length;
            } catch (error) {
              // Skip if module details can't be fetched
            }

            if (node.subModules && node.subModules.length > 0) {
              await countResources(node.subModules);
            }
          }
        };

        await countResources(tree);
