...
import { cleanMessage, extractImageUrls, removeImageUrlsFromText } from '../utils/chatUtils';
...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
  ...
  // Extract images from message text and combine with imageUrls field
  const imagesFromText = extractImageUrls(message.message);
  const allImageUrls = [
    ...(message.imageUrls || []),
    ...imagesFromText
  ].filter((url, index, self) => self.indexOf(url) === index); // Remove duplicates

  // Get cleaned message text without image URLs for display
  const displayMessage = allImageUrls.length > 0 
    ? removeImageUrlsFromText(message.message, allImageUrls)
    : message.message;
    ...
    <div className={`flex items-start space-x-3 ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`flex-1 min-w-full ${message.sender === 'user' ? 'flex justify-end' : 'flex justify-start'}`}>
        <div className={`group relative ${message.sender === 'user'
          ...
          {message.sender === 'bot' ? (
            <>
              {/* Image Carousel - Show before message text */}
            ...
            <>
              {/* Image Carousel for user messages too */}
              {allImageUrls.length > 0 && (
                <ImageCarousel
                  images={allImageUrls}
                  onImageClick={(index) => {
